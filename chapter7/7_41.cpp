...
public:
    //构造函数
    Sales_data(const std::string &s, unsigned u, double d) : book_num(s), units_sold(u), revenue(d) {std::cout << "三个参数" << std::endl;}
    Sales_data(): Sales_data(" ", 0, 0) {std::cout << "default" << std::endl;}
    Sales_data(const std::string &s): Sales_data(s, 0, 0) {std::cout << "一个参数" << std::endl;}
